...
$file = new FileUpload();
$elements = array(
  "sname" => $info[0]['sname'], "sage" => $info[0]['sage'], "ssex" => $info[0]['ssex'], "deptno" => $info[0]['dno'], "sdob" => $info[0]['sdob'], "simage" => ""
);
...
if (isset($_POST["btn_update"])) {
  if ($validator->validate($_POST)) {
    ...
    if (isset($_FILES['simage']['name'])) {
      if ($fileName = $file->doUploadRandom($_FILES['simage'], array('.jpg', '.png', '.jpeg'), 100000, 5, '../uploads')) {
        $flag = true;
      }
    }
    $data = array(
